...
let world=null,target=0,current=0,last=performance.now(),paused=reduced.matches,previousFocus=null,sceneFailed=false,manualFocus=null;
const focusToggle=document.createElement('button');focusToggle.id='focus-toggle';focusToggle.className='focus-replay';focusToggle.setAttribute('aria-pressed','true');focusToggle.textContent='SHOW THE WHOLE ROOM';$('.cafe-bottom').insertBefore(focusToggle,$('.story-connect'));
focusToggle.addEventListener('click',()=>{manualFocus=!(manualFocus??current*6>5.8);});
...
function tick(now){const dt=Math.min((now-last)/1000,.06);last=now;current=reduced.matches?target:current+(target-current)*(1-Math.exp(-dt*9));if(Math.abs(target-current)<.0001)current=target;const p=current*6;
  ...
  const cafeReady=Math.min(1,Math.max(0,(p-5.45)/.4));
  $('#conversation').style.setProperty('--cafe-ready',cafeReady);$('#conversation').inert=cafeReady<.8;
  $('#conversation').setAttribute('aria-hidden',String(p<4.7));
  if(p<5.4)manualFocus=null;
  const focused=manualFocus??p>5.8;if(focused!==previousFocus){previousFocus=focused;$('#cafe').classList.toggle('is-focused',focused);$('#focus-state').textContent=focused?'ONE CONVERSATION, IN FOCUS':'THE WORLD COMES BACK';$('#focus-copy').innerHTML=focused?'The room fades.<br><em>The connection stays.</em>':'A hundred distractions.<br><em>One conversation.</em>';focusToggle.setAttribute('aria-pressed',String(focused));focusToggle.textContent=focused?'SHOW THE WHOLE ROOM':'BRING THE CONVERSATION INTO FOCUS';}
  if(sceneFailed){$('#conversation').style.opacity=p>4.5?1:0;$('#conversation').style.clipPath='none';$('#world-error').hidden=p>4.5;}
  ...
  if(world)world.setProgress(current);requestAnimationFrame(tick);
...
$('#travel-toggle').addEventListener('click',()=>{goToProgress(0);world?.travel();});
