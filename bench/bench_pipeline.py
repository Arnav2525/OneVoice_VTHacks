...
def main() -> None:
    parser = build_argparser(
        "OneVoice pipeline stability benchmark", default_duration=600.0
    )
    ...
    run = setup_run("pipeline", config, args.output_dir)
    runner = PipelineBenchmarkRunner(
        name="pipeline", duration_s=args.duration, targets=targets
    )
